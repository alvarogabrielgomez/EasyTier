...
mod tests {
    use super::*;
    use crate::{
        common::stats_manager::StatsManager,
        tunnel::{mpsc::MpscTunnel, ring::create_ring_tunnel_pair},
    };

    // Adapted from the upstream #2476 test, which was written against the
    // post-split layout: its NoopPeerContext + network-name arguments become
    // this tree's AggregateTrafficMetrics, and foundation::time is tokio.
    ...
    async fn ingress_traffic_does_not_mask_failed_round_trips() {
        ...
        let mut pinger = PeerConnPinger::new(
            ...
            throughput.clone(),
            AggregateTrafficMetrics::control(Arc::new(StatsManager::new()), "test".to_owned()),
        );
